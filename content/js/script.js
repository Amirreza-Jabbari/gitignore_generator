// --- .gitignore templates for popular technologies ---
const gitignoreTemplates = {
  ...
  const techList = Object.keys(gitignoreTemplates);
  
  // --- State ---
  let selectedTechs = [];
  
  // --- DOM Elements ---
  const searchInput = document.querySelector('input[placeholder="Start typing to search..."]');
  const techTagsContainer = document.querySelector('.flex.gap-3.p-3.flex-wrap.pr-4');
  
  // Helper function to find button by its span text
  function getButtonBySpanText(text) {
  ...
  const generateBtn = getButtonBySpanText('Generate file');
  const downloadBtn = getButtonBySpanText('Download File');
  const copyBtn = getButtonBySpanText('Copy to Clipboard');
  
  // --- Helper: Create a tag element ---
  function createTag(name, removable = false) {
    const tag = document.createElement('div');
    tag.className = 'flex h-8 shrink-0 items-center justify-center gap-x-2 rounded-lg bg-[#29374C] pl-4 pr-4';
    const p = document.createElement('p');
    p.className = 'text-[#F8F9FB] text-sm font-medium leading-normal';
    p.textContent = name;
    tag.appendChild(p);
    ...
    if (removable) {
      ...
      x.textContent = '×';
      x.className = 'ml-2 cursor-pointer text-[#F4C753] font-bold';
      x.onclick = () => removeTech(name);
      tag.appendChild(x);
    }
    return tag;
  }
  
  // --- Render selected tech tags ---
  function renderTags() {
    techTagsContainer.innerHTML = '';
    selectedTechs.forEach(tech => {
      techTagsContainer.appendChild(createTag(tech, true));
    });
  }
  ...
  function showSuggestions(query) {
    ...
    document.querySelectorAll('.gitignore-suggestion').forEach(e => e.remove());
    if (!query) return;
  
    const matches = techList.filter(tech => tech.toLowerCase().includes(query.toLowerCase()) && !selectedTechs.includes(tech));
    matches.forEach(tech => {
      const suggestion = document.createElement('div');
      suggestion.className = 'gitignore-suggestion flex h-8 items-center px-4 bg-[#1D2A36] text-[#F8F9FB] rounded-lg cursor-pointer mt-1';
      ...
      searchInput.parentElement.appendChild(suggestion);
    });
  }
  ...
  function generateGitignore() {
    ...
    return selectedTechs.map(tech => `### ${tech}\n${gitignoreTemplates[tech] || ''}`).join('\n\n');
  }
  ...
  if (generateBtn) {
    generateBtn.addEventListener('click', () => {
      const content = generateGitignore();
      let output = document.getElementById('gitignore-output');
      if (!output) {
        output = document.createElement('textarea');
        output.id = 'gitignore-output';
        output.readOnly = true;
        output.className = 'w-full min-h-[200px] bg-[#1D2A36] text-[#F8F9FB] rounded-lg p-4 mt-4';
        generateBtn.parentElement.parentElement.appendChild(output);
      }
      output.value = content || 'No technologies selected.';
    });
  }
  ...
  if (copyBtn) {
    copyBtn.addEventListener('click', copyGitignore);
  }
